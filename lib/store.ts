      ...
      const newEntries = state.masterlistEntries.map((e) => {
        if (ids.includes(e.id)) {
          importedMembers.push({
            ...
            sex: e.sex,
            civilStatus: 'Single',
            address: e.type === 'Guest' ? 'San Fernando, Pampanga' : 'Unisite Subdivision, Barangay Del Pilar',
            city: 'San Fernando',
            province: 'Pampanga',
            zipCode: '2000',
            phone: 'N/A',
            email: e.email,
            membershipType: e.type === 'Student' ? 'Sponsored' : e.type === 'Guest' ? 'Voluntary' : 'Employed',
